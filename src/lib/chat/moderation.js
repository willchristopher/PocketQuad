...
const hardBlockPattern = /\b(fuck|fucking|shitty|shit|bitch|asshole|dick|pussy|cunt|slut|whore|motherfucker|bullshit)\b/i;
const hardHatePattern = /\b(?:i am|i'm|im|you are|you're|youre|he is|she is|they are|we are)\s+(?:a\s+)?(racist|sexist|homophobic|transphobic|bigot|white supremacist|neo-nazi)\b/i;
const hardThreatPattern = /\b(kill yourself|go die|i(?:'|’)ll kill you|i will kill you|beat you up|shoot you|stab you|i'm going to hurt you)\b/i;
...
function getModerationCache() {
    ...
    globalCache.__pocketquadModerationCache ??= new Map();
    return globalCache.__pocketquadModerationCache;
}
function hashMessage(content) {
    return crypto.createHash('sha256').update(content).digest('hex');
}
function getCachedModerationResult(messageId, content) {
...
function setCachedModerationResult(messageId, content, result) {
    const cache = getModerationCache();
    cache.set(messageId, {
...
function clearCachedModerationResult(messageId) {
    getModerationCache().delete(messageId);
}
async function runAiModeration(trimmed) {
    if (trimmed.length === 0) {
        return {
            allowed: false,
...
export async function moderateCampusChatMessage(content) {
    const trimmed = content.trim();
    return runAiModeration(trimmed);
}
export async function scanChannelMessagesForModeration(channelId) {
...
export async function reviewReportedMessage(messageId) {
