...
function parsePayload(value) {
    try {
        ...
        if (typeof parsed.sub !== 'string')
            return null;
        if (parsed.role !== 'STUDENT' && parsed.role !== 'FACULTY' && parsed.role !== 'ADMIN')
            return null;
...
export async function createRoleHintToken(supabaseUserId, role, now = new Date()) {
...
export async function verifyRoleHintToken(token, expectedSupabaseUserId, now = new Date()) {
    if (!token)
        return null;
    const secret = getRoleHintSecret();
    if (!secret)
    ...
    if (!payload)
        return null;
    if (payload.sub !== expectedSupabaseUserId)
        return null;
...
export function setRoleHintCookie(response, token) {
