...
type FacultyEventPermissionProfile = Parameters<typeof canManageBuilding>[0] & {
  id: string
  displayName: string
  universityId: string | null
}

type FacultyEventPayload = {
  title: string
  description: string
  date: string
  time: string
...
export function formatFacultyEventTimeLabel(time24: string) {
...
function formatEventMoment(date: Date, time: string) {
  return `${formatEventDate(date)} at ${formatFacultyEventTimeLabel(time)}`
}

export async function getFacultyEventOwner(userId: string) {
...
export async function resolveFacultyEventBuilding(
  profile: FacultyEventPermissionProfile,
  buildingId?: string | null,
) {
  if (!buildingId) {
    return null
...
function getFacultyEventRecipients(facultyId: string) {
...
function hasFacultyEventDetailsChanged(previous: FacultyEventSnapshot, next: FacultyEventSnapshot) {
  return (
...
function buildUpdatedEventMessage(previous: FacultyEventSnapshot, next: FacultyEventSnapshot) {
...
export async function notifyFavoritedStudentsAboutFacultyEvent(options: {
  facultyId: string
  actorName: string
  event: FacultyEventSnapshot
  type: 'NEW_EVENT' | 'EVENT_UPDATED' | 'EVENT_CANCELLED'
  previousEvent?: FacultyEventSnapshot
}) {
...
export function didFacultyEventChange(previous: FacultyEventSnapshot, next: FacultyEventSnapshot) {
