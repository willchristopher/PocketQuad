...
function LoginForm() {
    ...
    return (<div className="min-h-screen bg-[#050c18] text-white">
      <div className="grid min-h-screen lg:grid-cols-[minmax(0,1.1fr)_minmax(28rem,0.9fr)]">
        <section className="relative hidden overflow-hidden border-r border-white/10 lg:flex">
          ...
          <div className="relative z-10 flex min-h-screen flex-col px-10 pb-0 pt-10 xl:px-14 xl:pt-12">
            <Link href="/login" className="inline-flex w-fit items-center text-[2.4rem] font-semibold leading-none text-white transition-opacity hover:opacity-85">
            ...
            <div className="relative mt-auto flex min-h-[24rem] items-end justify-center overflow-hidden">
              ...
              <div className="animate-logo-emerge relative -mb-[24%] aspect-[843/1029] w-[18rem] opacity-0 xl:w-[22rem]">
                <Image src="/transparentlogo.png" alt="" fill priority aria-hidden="true" className="object-contain object-bottom drop-shadow-[0_34px_60px_rgba(0,82,255,0.24)]"/>
              </div>
            </div>
          </div>
        </section>
        ...
        <section className="relative flex min-h-screen items-center justify-center px-4 py-10 sm:px-6 lg:px-10 xl:px-14">
          ...
          <div className="relative z-10 w-full max-w-[30rem]">
            <div className="mb-8 lg:hidden">
              <Link href="/login" className="inline-flex items-center text-[2.2rem] font-semibold leading-none text-white transition-opacity hover:opacity-85">
                Pocket<span className="gradient-text">Quad</span>.
              </Link>
            </div>
              ...
              <div className="relative">
                ...
                <form className="mt-8 space-y-5" onSubmit={onSubmit}>
                  <AuthField label="University Email">
                    <AuthFieldShell icon={<Mail className="h-4 w-4"/>} className="border-white/10 bg-white/[0.03] hover:border-primary/30 focus-within:border-primary/45 focus-within:bg-white/[0.05] focus-within:ring-primary/20">
                      <input type="email" value={email} onChange={(event) => setEmail(event.target.value)} placeholder="you@university.edu" className="w-full bg-transparent text-sm text-white outline-none placeholder:text-slate-500" required autoComplete="email" disabled={submitting}/>
                    </AuthFieldShell>
                  </AuthField>

                  <AuthField label="Password" hint={<Link href="/forgot-password" className="text-xs font-medium text-sky-300 transition-colors hover:text-sky-200">
                        Forgot password?
                      </Link>}>
                    <AuthFieldShell icon={<Lock className="h-4 w-4"/>} className="border-white/10 bg-white/[0.03] hover:border-primary/30 focus-within:border-primary/45 focus-within:bg-white/[0.05] focus-within:ring-primary/20" trailing={<button type="button" onClick={() => setShowPassword((value) => !value)} className="text-slate-400 transition-colors hover:text-white" disabled={submitting} aria-label={showPassword ? 'Hide password' : 'Show password'}>
                          {showPassword ? (<EyeOff className="h-4 w-4"/>) : (<Eye className="h-4 w-4"/>)}
                        </button>}>
                      <input type={showPassword ? 'text' : 'password'} value={password} onChange={(event) => setPassword(event.target.value)} placeholder="Enter your password" className="w-full bg-transparent text-sm text-white outline-none placeholder:text-slate-500" required autoComplete="current-password" disabled={submitting}/>
                    </AuthFieldShell>
                  </AuthField>
                  ...
                  <label className="inline-flex cursor-pointer items-center gap-3 text-sm text-slate-300">
                    <input type="checkbox" checked={rememberMe} onChange={(event) => setRememberMe(event.target.checked)} disabled={submitting} className="h-4 w-4 rounded border-slate-500 bg-transparent text-primary focus:ring-primary/20"/>
