...
import { ArrowRight, Eye, EyeOff, GraduationCap, Lock, Mail, Sparkles, User, } from 'lucide-react';
import Link from 'next/link';
import { useRouter } from 'next/navigation';
import { AppBrand, AuthCard, AuthField, AuthFieldShell, AuthMessage, AuthShell, } from '@/components/auth/AuthShell';
import { Badge } from '@/components/ui/badge';
import { Button } from '@/components/ui/button';
import { ApiClientError, apiRequest } from '@/lib/api/client';
import { useAuth } from '@/lib/auth/context';
function getFacultyStepNumber(step) {
    if (step === 'email')
        return 1;
    if (step === 'code')
        return 2;
...
export default function RegisterPage() {
    ...
    return (<AuthShell>
      <div className="space-y-6">
        ...
        <AuthCard badge={isFaculty ? 'Faculty Activation' : 'Student Sign Up'} title={title} description={description} footer={<p className="text-center text-sm text-muted-foreground">
          ...
          <form className="space-y-5" onSubmit={onSubmit}>
            <div className="grid grid-cols-2 gap-3">
              <button type="button" onClick={() => handleRoleChange('STUDENT')} className={`rounded-2xl border p-4 text-left transition-all ${role === 'STUDENT'
            ? 'border-primary/25 bg-primary/[0.06] shadow-accent'
            : 'border-border/70 bg-card/75 hover:border-primary/15 hover:shadow-surface'}`} disabled={submitting || resendingCode}>
              ...
              <button type="button" onClick={() => handleRoleChange('FACULTY')} className={`rounded-2xl border p-4 text-left transition-all ${role === 'FACULTY'
            ? 'border-primary/25 bg-primary/[0.06] shadow-accent'
            : 'border-border/70 bg-card/75 hover:border-primary/15 hover:shadow-surface'}`} disabled={submitting || resendingCode}>
            ...
            {isFaculty ? (<div className="space-y-3">
                <div className="grid grid-cols-3 gap-2">
                  {[1, 2, 3].map((stepNumber) => {
                const currentStep = getFacultyStepNumber(facultyStep);
                const active = stepNumber === currentStep;
                const completed = stepNumber < currentStep;
                return (<div key={stepNumber} className={`rounded-full border px-3 py-2 text-center text-[11px] font-semibold uppercase tracking-[0.14em] ${active
            ...
            {isStudent && studentStep === 'code' ? (<AuthMessage>
                Enter the one-time passcode sent to your university email to finish creating your account.
              </AuthMessage>) : null}

            {isStudent && studentStep === 'details' ? (<div className="grid gap-4 sm:grid-cols-2">
                <AuthField label="First Name" error={fieldErrors.firstName?.[0]}>
                  <AuthFieldShell icon={<User className="h-4 w-4"/>} invalid={!!fieldErrors.firstName?.[0]}>
                    <input type="text" value={firstName} onChange={(event) => setFirstName(event.target.value)} placeholder="Alex" className="w-full bg-transparent text-sm text-foreground outline-none placeholder:text-muted-foreground/60" required disabled={submitting}/>
                  </AuthFieldShell>
                </AuthField>

                <AuthField label="Last Name" error={fieldErrors.lastName?.[0]}>
                  <AuthFieldShell invalid={!!fieldErrors.lastName?.[0]}>
                    <input type="text" value={lastName} onChange={(event) => setLastName(event.target.value)} placeholder="Johnson" className="w-full bg-transparent text-sm text-foreground outline-none placeholder:text-muted-foreground/60" required disabled={submitting}/>
                  </AuthFieldShell>
                </AuthField>
              </div>) : null}
            ...
            <AuthField label="University Email" error={fieldErrors.email?.[0]}>
              <AuthFieldShell icon={<Mail className="h-4 w-4"/>} invalid={!!fieldErrors.email?.[0]}>
                <input type="email" value={email} onChange={(event) => setEmail(event.target.value.toLowerCase())} placeholder="you@university.edu" className="w-full bg-transparent text-sm text-foreground outline-none placeholder:text-muted-foreground/60" required autoComplete="email" disabled={submitting ||
            (isFaculty && facultyStep !== 'email') ||
            (isStudent && studentStep !== 'details')}/>
              </AuthFieldShell>
            </AuthField>

            {detectedUniversity || checkingDomain ? (<AuthMessage>
                {checkingDomain ? ('Checking your university domain...') : (<>
                    You&apos;ll be joining <span className="font-semibold text-foreground">{detectedUniversity}</span>.
                  </>)}
              </AuthMessage>) : null}

            {((isFaculty && facultyStep === 'code') || (isStudent && studentStep === 'code')) ? (<AuthField label="One-Time Passcode" error={fieldErrors.code?.[0]} hint={<button type="button" onClick={isFaculty ? resendFacultyCode : resendStudentCode} className="text-xs font-medium text-primary transition-colors hover:text-primary/80 disabled:opacity-60" disabled={submitting || resendingCode}>
            ...
            {((isStudent && studentStep === 'details') || (isFaculty && facultyStep === 'password')) ? (<AuthField label="Password" error={fieldErrors.password?.[0]} hint={isStudent ? <Badge variant="subtle">Minimum 8 characters</Badge> : undefined}>
            ...
            <Button type="submit" variant="gradient" size="xl" className="w-full gap-2" disabled={submitting || resendingCode}>
              {submitLabel}
