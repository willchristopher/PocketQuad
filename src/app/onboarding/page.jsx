...
export default function OnboardingPage() {
    ...
    const studentSteps = ['welcome', 'faculty', 'buildings', 'clubs', 'theme'];
    const facultySteps = ['welcome', 'officeHours', 'theme'];
    const steps = isFaculty ? facultySteps : studentSteps;
    const [currentStepIdx, setCurrentStepIdx] = React.useState(0);
    const [direction, setDirection] = React.useState('forward');
    const [favoriteFacultyIds, setFavoriteFacultyIds] = React.useState([]);
    ...
    return (<OnboardingShell currentStep={currentStepIdx} totalSteps={steps.length} footer={<StepFooter onBack={goBack} onSkip={skipOnboarding} onNext={isLastStep ? finishOnboarding : goNext} disableBack={currentStepIdx === 0 || submitting} disableNext={submitting} isLastStep={isLastStep} submitting={submitting}/>}>
      <StepCard direction={direction}>
        {currentStep === 'welcome' ? (<WelcomeStep name={profile.firstName} role={profile.role}/>) : null}
        {currentStep === 'faculty' ? (<FacultyStep facultyList={facultyList} selected={favoriteFacultyIds} onToggle={(id) => setFavoriteFacultyIds((previous) => previous.includes(id) ? previous.filter((facultyId) => facultyId !== id) : [...previous, id])} searchQuery={searchQuery} onSearch={setSearchQuery}/>) : null}
        {currentStep === 'buildings' ? (<BuildingsStep buildings={buildingList} enabled={buildingAlerts} onToggle={setBuildingAlerts} selected={selectedBuildingIds} onSelect={(id) => setSelectedBuildingIds((previous) => previous.includes(id) ? previous.filter((buildingId) => buildingId !== id) : [...previous, id])} searchQuery={searchQuery} onSearch={setSearchQuery}/>) : null}
        {currentStep === 'clubs' ? (<ClubsStep clubs={clubList} selected={selectedClubIds} onToggle={(id) => setSelectedClubIds((previous) => previous.includes(id) ? previous.filter((clubId) => clubId !== id) : [...previous, id])} searchQuery={searchQuery} onSearch={setSearchQuery}/>) : null}
        {currentStep === 'theme' ? (<ThemeStep selected={selectedTheme} onSelect={setSelectedTheme}/>) : null}
        {currentStep === 'officeHours' ? (<OfficeHoursStep officeHours={officeHours} onChange={setOfficeHours}/>) : null}
      </StepCard>
    </OnboardingShell>);
}
...
function FacultyStep({ facultyList, selected, onToggle, searchQuery, onSearch, }) {
    const filtered = facultyList.filter((faculty) => faculty.name.toLowerCase().includes(searchQuery.toLowerCase()) ||
        (faculty.department ?? '').toLowerCase().includes(searchQuery.toLowerCase()));
    return (<div className="space-y-5">
      <StepHeader badge="Favorite Faculty" icon={<Heart className="h-5 w-5"/>} title="Choose faculty to follow" description="Pick professors you want quick access to for updates, office hours, and contact info."/>
      ...
      {selected.length > 0 ? (<p className="text-sm text-muted-foreground">{selected.length} faculty selected</p>) : null}
    </div>);
}
function BuildingsStep({ buildings, enabled, onToggle, selected, onSelect, searchQuery, onSearch, }) {
    const filtered = buildings.filter((building) => building.name.toLowerCase().includes(searchQuery.toLowerCase()) ||
        (building.code ?? '').toLowerCase().includes(searchQuery.toLowerCase()));
    return (<div className="space-y-5">
      <StepHeader badge="Building Alerts" icon={<Building2 className="h-5 w-5"/>} title="Keep an eye on campus spaces" description="Turn on alerts for building closures, service changes, and the places you depend on most."/>

      <StepToggle enabled={enabled} onToggle={onToggle} title="Enable building closure alerts" description="Receive updates when important buildings close or change operating status."/>
      ...
      {enabled ? (<>
          ...
          {selected.length > 0 ? (<p className="text-sm text-muted-foreground">{selected.length} buildings selected</p>) : null}
        </>) : null}
    </div>);
}
function ClubsStep({ clubs, selected, onToggle, searchQuery, onSearch, }) {
    const filtered = clubs.filter((club) => club.name.toLowerCase().includes(searchQuery.toLowerCase()) ||
        (club.category ?? '').toLowerCase().includes(searchQuery.toLowerCase()));
    return (<div className="space-y-5">
      <StepHeader badge="Club Interests" icon={<Users className="h-5 w-5"/>} title="Pick the communities that matter" description="Following clubs makes it easier to see activity, events, and organizations you may want to join."/>
      ...
      {selected.length > 0 ? (<p className="text-sm text-muted-foreground">{selected.length} clubs selected</p>) : null}
    </div>);
}
...
function OfficeHoursStep({ officeHours, onChange, }) {
    ...
    const updateSlot = (index, field, value) => {
        onChange(officeHours.map((slot, slotIndex) => slotIndex === index ? { ...slot, [field]: value } : slot));
    };
    return (<div className="space-y-5">
      <StepHeader badge="Office Hours" icon={<Clock className="h-5 w-5"/>} title="Set your weekly availability" description="Choose the days and times students can expect to find you, then add the location or meeting mode."/>

      <div className="flex flex-wrap gap-2">
        {DAYS.map((day, index) => {
            const hasSlot = officeHours.some((slot) => slot.dayOfWeek === index);
            return (<DayChip key={day} selected={hasSlot} onClick={() => !hasSlot && addSlot(index)}>
              {day}
            </DayChip>);
        })}
      </div>
