...
export default function ClubsPage() {
    ...
    return (<div className="space-y-6">
      ...
      <section className="grid gap-4 md:grid-cols-2">
        ...
        {!loading && clubs.map((club, index) => (<article key={club.id} className="rounded-2xl border border-border/60 bg-card p-5 hover-lift animate-in-up" style={{ animationDelay: `${0.04 * (index + 1)}s` }}>
            ...
            {club.websiteUrl && (<a href={club.websiteUrl} target="_blank" rel="noreferrer" className="mt-4 inline-flex items-center gap-1.5 rounded-xl border border-border/60 px-3 py-2 text-xs font-semibold transition-colors hover:bg-muted/35">
      ...
      {!loading && clubs.length === 0 && (<section className="rounded-2xl border border-dashed border-border/60 bg-card p-10 text-center text-sm text-muted-foreground animate-in-up">
