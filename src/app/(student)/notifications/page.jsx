...
const filters = ['All', 'Unread', 'Announcements', 'Events', 'Deadlines', 'System'];
function typeToFilter(type) {
    if (type === 'ANNOUNCEMENT')
        return 'Announcements';
    if (type === 'NEW_EVENT' || type === 'EVENT_CANCELLED')
        return 'Events';
    if (type === 'DEADLINE')
        return 'Deadlines';
    return 'System';
...
export default function NotificationsPage() {
    ...
    return (<div className="space-y-6">
      <div className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between animate-in-up">
        ...
        {unreadCount > 0 && (<button onClick={() => void markAllRead()} disabled={updating} className="inline-flex items-center gap-1.5 rounded-xl border border-border/60 bg-card px-3.5 py-2 text-sm font-semibold transition-all duration-200 hover:bg-muted/35 disabled:opacity-70">
      ...
      <section className="flex flex-wrap gap-2 animate-in-up stagger-1">
        {filters.map((filter) => (<button key={filter} onClick={() => setActiveFilter(filter)} className={cn('rounded-full border px-3 py-1.5 text-xs font-semibold transition-all duration-200', activeFilter === filter
      ...
      <section className="space-y-2.5">
        ...
        {!loading && filtered.map((item, index) => (<button key={item.id} onClick={() => {
                if (!item.read) {
                    void markOneRead(item.id);
                }
            }} className={cn('w-full rounded-2xl border p-4 text-left transition-all duration-200 hover-lift animate-in-up', item.read
                ? 'border-border/60 bg-card hover:bg-muted/25'
                : 'border-primary/25 bg-primary/[0.04] hover:bg-primary/[0.07]')} style={{ animationDelay: `${0.035 * (index + 1)}s` }}>
