...
export default function ChatroomPage() {
    ...
    const loadRoom = React.useCallback(async () => {
        ...
        setError(null);
        try {
            const result = await apiRequest('/api/channels/campus-room');
            setRoom(result);
        }
        catch (err) {
    ...
    const loadMessages = React.useCallback(async (channelId) => {
        setLoadingMessages(true);
        setError(null);
        try {
            const result = await apiRequest(`/api/channels/${channelId}/messages?limit=80`);
    ...
    const scheduleMessagesReload = React.useCallback((channelId, delayMs = 300) => {
    ...
    const sendMessage = async () => {
        ...
        if (!trimmed || !room?.id || sending)
            return;
        setSending(true);
        setError(null);
        try {
            const created = await apiRequest(`/api/channels/${room.id}/messages`, {
                method: 'POST',
                body: {
                    content: trimmed,
                },
    ...
    const reportMessage = async (messageId) => {
        ...
        setError(null);
        try {
            const result = await apiRequest(`/api/messages/${messageId}/report`, {
                method: 'POST',
                body: { reason },
            });
    ...
    return (<div className="overflow-hidden rounded-2xl border border-border/60 bg-card animate-in-up">
      <section className="flex min-h-[70vh] flex-col">
        ...
        <div className="flex-1 space-y-3 overflow-y-auto p-4 custom-scrollbar">
          ...
          {!loadingRoom && loadingMessages && (<p className="text-xs text-muted-foreground">Loading messages...</p>)}

          {!loadingMessages && messages.length === 0 && (<p className="rounded-xl border border-dashed border-border/60 bg-muted/10 p-4 text-sm text-muted-foreground">
          ...
          {messages.map((message, index) => {
            ...
            return (<div key={message.id} className={cn('rounded-xl border p-3 hover-lift animate-in-up', isOwn ? 'border-primary/30 bg-primary/5' : 'border-border/50 bg-muted/10')} style={{ animationDelay: `${0.02 * (index + 1)}s` }}>
                ...
                <p className={cn('mt-1 text-sm text-muted-foreground', message.isDeleted && 'italic text-muted-foreground/80')}>
                  {message.content}
                </p>
              </div>);
        })}
