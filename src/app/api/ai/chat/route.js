...
function buildSystemPrompt(universityContext, userInfo) {
...
export async function POST(request) {
    try {
        ...
        const universityContext = await gatherUniversityContext(profile.universityId, profile.id, {
            ...contextPlan,
        ...
        if (error instanceof Error) {
            ...
            if (msg.includes('rate') || msg.includes('limit') || msg.includes('429')) {
                return NextResponse.json({ error: 'The AI service is temporarily rate-limited. Please wait a moment and try again.' }, { status: 429 });
            }
            // Model not found / bad request
            if (msg.includes('model') || msg.includes('400')) {
                return NextResponse.json({ error: 'AI model configuration error. Please contact an administrator.' }, { status: 502 });
            }
