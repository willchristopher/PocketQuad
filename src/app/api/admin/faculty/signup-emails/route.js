...
function splitLocalPart(localPart) {
...
export async function GET(request) {
    try {
        ...
        if (!isOwner && requestedUniversityId && requestedUniversityId !== profile.universityId) {
            throw new ApiError(403, 'You can only view faculty signup emails for your own university');
        }
...
export async function POST(request) {
    try {
        ...
        if (!isOwner && payload.universityId !== profile.universityId) {
            throw new ApiError(403, 'You can only add faculty signup emails for your own university');
        }
