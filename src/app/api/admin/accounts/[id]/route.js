...
import { dedupePermissions, getDefaultPermissionsForAccessLevel, } from '@/lib/auth/portalPermissions';
...
function ensureScopedToUniversity(isOwner, actorUniversityId, targetUniversityId) {
    if (isOwner)
        return;
...
function getNextPermissions(args) {
    if (args.requestedPermissions) {
        return dedupePermissions([
            ...
            ...(args.nextAccessLevel ? ['ADMIN_PORTAL_ACCESS'] : []),
        ]);
    }
    if (args.accessLevelChanged) {
        if (!args.nextAccessLevel)
            return [];
...
export async function PATCH(request, context) {
    try {
        ...
        if (target.id === profile.id &&
            ...
            throw new ApiError(400, 'You cannot change your own role or permission set from this screen');
        }
        const nextAccessLevel = payload.accessLevel !== undefined ? payload.accessLevel : target.adminAccessLevel;
        ...
        if (payload.canPublishCampusAnnouncements !== undefined) {
            nextPermissions = payload.canPublishCampusAnnouncements
                ...
                : nextPermissions.filter((permission) => permission !== 'CAN_PUBLISH_ANNOUNCEMENTS');
        }
        const canPublishCampusAnnouncements = payload.canPublishCampusAnnouncements ??
