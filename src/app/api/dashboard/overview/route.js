...
import { getActiveAnnouncementWhere, listUniversityAnnouncements, purgeExpiredAnnouncements } from '@/lib/server/announcements';
...
export async function GET() {
    try {
        ...
        const favoriteFacultyPromise = prisma.facultyFavorite.findMany({
            ...
            const studentAvailability = getStudentFacingFacultyAvailability(row.faculty.availabilityStatus, row.faculty.availabilityNote, row.faculty.officeHourSlots);
            return {
                ...row.faculty,
            ...
            return rows.map((row) => {
                ...
                const studentAvailability = getStudentFacingFacultyAvailability(availability.status, availability.note, row.faculty.officeHourSlots);
                return {
                    ...row.faculty,
                    officeHours: summarizeFacultyOfficeHours(row.faculty.officeHourSlots),
        ...
        const [upcomingEvents, upcomingDeadlines, serviceSnapshot, quickLinks, clubSnapshot, favoriteFaculty, campusNews, pinnedBuildings, pinnedClubs,] = await Promise.all([
