...
import { getAnnouncementAudienceLabel, listUniversityAnnouncements, } from '@/lib/server/announcements';
...
export async function GET() {
    try {
        ...
        return successResponse({
            ...
            permissions: {
                canPublishCampus,
                canPublishBuildings: schemaSupportsScopedAnnouncements &&
...
export async function POST(request) {
    try {
        ...
        if (!schemaSupportsScopedAnnouncements && payload.scope !== 'CAMPUS') {
            throw new ApiError(409, 'Building and service announcements are not available until the latest database schema is applied');
        }
        ...
        try {
            ...
            if (payload.scope !== 'CAMPUS') {
                throw new ApiError(409, 'Scoped announcements are not available until the latest database schema is applied');
            }
            announcement = await prisma.announcement.create({
                data: {
                    title: payload.title,
        ...
        const recipients = announcement.scope === 'BUILDING' && announcement.buildingId
        ...
        if (recipients.length > 0) {
            await prisma.notification.createMany({
                data: recipients.map((recipient) => ({
                    ...
                    actionUrl: announcement.linkUrl ??
