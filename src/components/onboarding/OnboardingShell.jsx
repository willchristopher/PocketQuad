...
export function SelectionRow({ selected, leading, title, description, meta, trailing, onClick, className, }) {
    const content = (<>
    ...
    if (!onClick) {
        return (<div className={cn(selectionRowVariants({ selected }), className)}>
        {content}
      </div>);
    }
    return (<button type="button" onClick={onClick} className={cn(selectionRowVariants({ selected }), className)}>
      {content}
    </button>);
}
...
export function StepToggle({ enabled, onToggle, title, description, }) {
    return (<button type="button" onClick={() => onToggle(!enabled)} className={cn('flex w-full items-center justify-between gap-4 rounded-2xl border p-4 text-left transition-all duration-200', enabled
      ...
      <div className={cn('relative h-7 w-12 rounded-full transition-colors duration-200', enabled ? 'bg-primary' : 'bg-muted-foreground/25')}>
        <span className={cn('absolute top-1 h-5 w-5 rounded-full bg-white shadow-sm transition-all duration-200', enabled ? 'left-6' : 'left-1')}/>
      </div>
    </button>);
}
...
export function StepFooter({ onBack, onSkip, onNext, disableBack, disableNext, isLastStep, submitting, }) {
    return (<div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
      <Button type="button" variant="surface" size="xl" onClick={onBack} disabled={disableBack} className="justify-center sm:justify-start">
        Back
      </Button>
      <Button type="button" variant="ghost" size="xl" onClick={onSkip} disabled={submitting} className="order-3 text-muted-foreground sm:order-2">
        Skip for now
      </Button>
      <Button type="button" variant="gradient" size="xl" onClick={onNext} disabled={disableNext} className="order-2 sm:order-3">
        {submitting ? 'Finishing...' : isLastStep ? "Let's go" : 'Next'}
      </Button>
    </div>);
}
export function StatusPill({ variant, children, }) {
    return (<span className={cn(statusPillVariants({ variant }))}>
      {children}
    </span>);
}
const statusPillVariants = cva('inline-flex items-center rounded-full px-2.5 py-1 text-[10px] font-semibold uppercase tracking-[0.12em]', {
    variants: {
        variant: {
