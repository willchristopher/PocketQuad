        ...
        const loadEntries = async () => {
            ...
            try {
                const [eventsData, calendarData, deadlinesData] = await Promise.all([
                    ...
                    apiRequest(`/api/calendar?start=${encodeURIComponent(start)}&end=${encodeURIComponent(end)}`),
    ...
    return (<section className="overflow-hidden rounded-2xl border border-border/60 bg-card animate-in-up">
      <div className="flex flex-col gap-3 border-b border-border/60 bg-muted/10 p-4 md:flex-row md:items-center md:justify-between md:p-5">
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center overflow-hidden rounded-xl border border-border/60 bg-card/80">
            <button onClick={() => setCurrentMonth(subMonths(currentMonth, 1))} className="p-2 transition-colors hover:bg-muted/40" aria-label="Previous month">
              <ChevronLeft className="h-4 w-4"/>
            </button>
            <button onClick={() => setCurrentMonth(new Date())} className="border-x border-border/60 px-3 py-1.5 text-xs font-semibold transition-colors hover:bg-muted/40">
              Today
            </button>
            <button onClick={() => setCurrentMonth(addMonths(currentMonth, 1))} className="p-2 transition-colors hover:bg-muted/40" aria-label="Next month">
        ...
        <div className="flex gap-1.5 overflow-x-auto pb-1">
          {filterOptions.map((option) => (<button key={option} onClick={() => setActiveFilter(option)} className={cn('whitespace-nowrap rounded-full border px-3 py-1.5 text-xs font-semibold transition-all duration-200', activeFilter === option
      ...
      <div className="grid min-h-[520px] grid-cols-7 auto-rows-fr">
        {monthDays.map((day) => {
            ...
            return (<button key={day.toISOString()} onClick={() => setSelectedDate(day)} className={cn('min-h-[88px] border-b border-r border-border/30 p-1.5 text-left transition-all duration-200 hover:bg-muted/30', !currentMonthDay && 'opacity-35', isToday(day) && 'bg-primary/[0.04]')}>
              <span className={cn('mb-1 inline-flex h-6 w-6 items-center justify-center rounded-lg text-xs font-bold', isToday(day) ? 'bg-primary text-primary-foreground' : 'text-foreground')}>
                {format(day, 'd')}
              </span>

              <div className="space-y-1">
                {loading ? (<p className="px-1 text-[10px] text-muted-foreground">...</p>) : (<>
                    {dayEntries.slice(0, 2).map((entry) => (<div key={entry.id} className={cn('truncate rounded-md border px-1.5 py-0.5 text-[10px] font-semibold animate-in-up', entryColor[entry.kind])}>
                        {entry.title}
                      </div>))}
                    {dayEntries.length > 2 && (<p className="px-1 text-[10px] font-medium text-muted-foreground">+{dayEntries.length - 2} more</p>)}
                  </>)}
              </div>
            </button>);
        })}
      ...
      {selectedDate && (<Dialog open={Boolean(selectedDate)} onOpenChange={() => setSelectedDate(null)}>
          <DialogContent className="rounded-2xl">
            ...
            <div className="space-y-2">
              {selectedEntries.map((entry, index) => (<div key={entry.id} className={cn('rounded-xl border p-3 animate-in-up', entryColor[entry.kind])} style={{ animationDelay: `${0.03 * (index + 1)}s` }}>
