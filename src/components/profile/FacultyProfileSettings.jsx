...
import { Accessibility, BellRing, Briefcase, Building2, CalendarDays, Clock3, Mail, MapPin, Megaphone, Plus, Save, Sparkles, Tag, UserRound, X, } from 'lucide-react';
...
export function FacultyProfileSettings() {
    ...
    const getErrorMessage = React.useCallback((value, fallback) => value instanceof ApiClientError ? value.message : fallback, []);
    const loadFacultyProfile = React.useCallback(async () => {
        setLoading(true);
        setError(null);
        try {
            const data = await apiRequest('/api/faculty/me');
            setFaculty(data);
            setDraft({
    ...
    React.useEffect(() => {
        ...
        const selectedStillManaged = buildingData.managedBuildings.some((building) => building.id === selectedBuildingId);
        if (!selectedStillManaged) {
            setSelectedBuildingId(buildingData.managedBuildings[0]?.id ?? '');
        }
    }, [buildingData, selectedBuildingId]);
    const selectedBuilding = React.useMemo(() => buildingData?.managedBuildings.find((building) => building.id === selectedBuildingId) ?? null, [buildingData, selectedBuildingId]);
    React.useEffect(() => {
    ...
    const filteredBuildings = React.useMemo(() => {
        ...
        return (buildingData?.availableBuildings ?? []).filter((building) => `${building.name} ${building.type} ${building.address}`.toLowerCase().includes(query));
    ...
    const onSubmit = async (event) => {
        ...
        setSuccess(null);
        try {
            const updated = await apiRequest('/api/faculty/me', {
                method: 'PATCH',
                body: draft,
            });
            setFaculty(updated);
    ...
    return (<div className="space-y-6">
      ...
      {success && (<p className="rounded-2xl border border-emerald-500/30 bg-emerald-500/10 px-4 py-3 text-sm font-medium text-emerald-700 dark:text-emerald-300">
          {success}
        </p>)}
      ...
      <Tabs defaultValue="profile" className="space-y-6">
          ...
          <div className="grid gap-6 xl:grid-cols-[minmax(0,1.35fr)_360px]">
            <form onSubmit={onSubmit} className="space-y-6">
              <section className="rounded-[24px] border border-border/60 bg-card p-5 md:p-6">
                ...
                <div className="grid gap-4 md:grid-cols-2">
                  <label className="space-y-2 text-sm font-medium">
                    <span>Name</span>
                    <Input value={draft.displayName} onChange={(event) => setDraft((current) => ({ ...current, displayName: event.target.value }))} placeholder="Dr. Maya Thompson" required/>
                  </label>

                  <label className="space-y-2 text-sm font-medium">
                    <span>Title</span>
                    <Input value={draft.title} onChange={(event) => setDraft((current) => ({ ...current, title: event.target.value }))} placeholder="Associate Professor" required/>
                  </label>

                  <label className="space-y-2 text-sm font-medium">
                    <span>Department</span>
                    <Input value={draft.department} onChange={(event) => setDraft((current) => ({ ...current, department: event.target.value }))} placeholder="Computer Science" required/>
                  </label>

                  <label className="space-y-2 text-sm font-medium">
                    <span>Office location</span>
                    <Input value={draft.officeLocation} onChange={(event) => setDraft((current) => ({ ...current, officeLocation: event.target.value }))} placeholder="Engineering Hall 314" required/>
                  </label>

                  <label className="space-y-2 text-sm font-medium md:col-span-2">
                    <span>Phone (optional)</span>
                    <Input value={draft.phone} onChange={(event) => setDraft((current) => ({ ...current, phone: event.target.value }))} placeholder="(555) 555-0148"/>
                  </label>
                </div>
              </section>
              ...
              <section className="rounded-[24px] border border-border/60 bg-card p-5 md:p-6">
                ...
                <div className="space-y-4">
                  <label className="space-y-2 text-sm font-medium">
                    <span>Short bio</span>
                    <Textarea value={draft.bio} onChange={(event) => setDraft((current) => ({ ...current, bio: event.target.value }))} placeholder="I help students with registration planning, degree audits, internships, and academic appeals." className="min-h-36 resize-none"/>
                  </label>
            ...
            if (event.key === 'Enter' || event.key === ',') {
                ...
                addTag();
                    ...
                    <div className="flex min-h-14 flex-wrap gap-2 rounded-2xl border border-dashed border-border/60 bg-muted/10 p-3">
                      {draft.tags.length === 0 ? (<p className="text-sm text-muted-foreground">No tags yet. Add the kinds of questions or resources students should come to you for.</p>) : (draft.tags.map((tag) => (<Badge key={tag} variant="secondary" className="flex items-center gap-1 rounded-full px-3 py-1 text-xs font-medium">
                            {tag}
            ...
            <aside className="space-y-6">
              <section className="rounded-[24px] border border-border/60 bg-card p-5 md:p-6">
                ...
                {loading ? (<p className="text-sm text-muted-foreground">Loading profile preview...</p>) : (<div className="space-y-4">
                ...
                : 'Availability status unavailable'}
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="flex flex-wrap gap-2">
                        {draft.tags.length === 0 ? (<p className="text-sm text-muted-foreground">No routing tags yet.</p>) : (draft.tags.map((tag) => (<Badge key={tag} variant="outline" className="rounded-full px-3 py-1 text-xs">
                              {tag}
                            </Badge>)))}
                      </div>
                    </div>
        ...
        <TabsContent value="building-management" className="mt-0 space-y-6">
          {buildingError && (<p className="rounded-2xl border border-red-500/30 bg-red-500/10 px-4 py-3 text-sm font-medium text-red-700 dark:text-red-300">
              {buildingError}
            </p>)}

          {buildingSuccess && (<p className="rounded-2xl border border-emerald-500/30 bg-emerald-500/10 px-4 py-3 text-sm font-medium text-emerald-700 dark:text-emerald-300">
              {buildingSuccess}
            </p>)}
          ...
          <section className="rounded-[24px] border border-border/60 bg-card p-5 md:p-6">
            <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
              ...
              <Input value={buildingSearch} onChange={(event) => setBuildingSearch(event.target.value)} placeholder="Search buildings" className="md:max-w-xs"/>
            </div>

            {buildingsLoading ? (<p className="mt-5 text-sm text-muted-foreground">Loading building access...</p>) : (<div className="mt-5 grid gap-3 md:grid-cols-2 xl:grid-cols-3">
                {filteredBuildings.map((building) => (<article key={building.id} className="rounded-2xl border border-border/60 bg-muted/10 p-4">
                    ...
                    <div className="mt-4 flex items-center gap-2">
                      {building.isManaged ? (<>
                          <Button type="button" size="sm" className="rounded-xl" onClick={() => setSelectedBuildingId(building.id)}>
                            Manage
                          </Button>
                          <Button type="button" size="sm" variant="outline" className="rounded-xl" onClick={() => void releaseBuilding(building.id)} disabled={releasingBuildingId === building.id}>
                            {releasingBuildingId === building.id ? 'Removing...' : 'Remove'}
                          </Button>
                        </>) : (<Button type="button" size="sm" className="rounded-xl" onClick={() => void claimBuilding(building.id)} disabled={claimingBuildingId === building.id}>
          ...
          {!selectedBuilding ? (<section className="rounded-[24px] border border-dashed border-border/60 bg-card p-8 text-center text-sm text-muted-foreground">
              Add at least one building above to open the building management workspace.
            </section>) : (<div className="grid gap-6 xl:grid-cols-[280px_minmax(0,1fr)]">
              <aside className="space-y-3">
                {(buildingData?.managedBuildings ?? []).map((building) => (<button key={building.id} type="button" onClick={() => setSelectedBuildingId(building.id)} className={cn('w-full rounded-2xl border p-4 text-left transition-colors', selectedBuildingId === building.id
              ...
              <div className="space-y-6">
                ...
                <div className="grid gap-6 xl:grid-cols-2">
                  ...
                  <div className="grid gap-4 md:grid-cols-2">
                    <label className="space-y-2 text-sm font-medium md:col-span-2">
                      ...
                      <Input value={buildingDraft.operatingHours} onChange={(event) => setBuildingDraft((current) => ({ ...current, operatingHours: event.target.value }))} placeholder="Mon-Fri 7:00 AM - 9:00 PM"/>
                    </label>

                    <label className="space-y-2 text-sm font-medium">
                      <span>Status</span>
                      <select value={buildingDraft.operationalStatus} onChange={(event) => setBuildingDraft((current) => ({
                ...current,
                operationalStatus: event.target.value,
                    ...
                    <label className="space-y-2 text-sm font-medium">
                      ...
                      <Input value={buildingDraft.operationalNote} onChange={(event) => setBuildingDraft((current) => ({ ...current, operationalNote: event.target.value }))} placeholder="Elevator maintenance on east side"/>
                    </label>

                    <label className="space-y-2 text-sm font-medium md:col-span-2">
                      <span>Accessibility notes</span>
                      <Textarea value={buildingDraft.accessibilityNotes} onChange={(event) => setBuildingDraft((current) => ({ ...current, accessibilityNotes: event.target.value }))} placeholder="Automatic doors active at north entrance. Ramp access temporarily rerouted through the plaza entrance." className="min-h-32 resize-none"/>
                    </label>
                  </div>

                  <div className="mt-5 flex flex-wrap items-center gap-3">
                    <Button type="submit" className="rounded-2xl px-5" disabled={savingBuildingId === selectedBuilding.id}>
                      <Save className="mr-2 h-4 w-4"/>
                      {savingBuildingId === selectedBuilding.id ? 'Saving...' : 'Save building details'}
                    </Button>
                    {selectedBuilding.operationalUpdatedAt && (<p className="text-sm text-muted-foreground">
                  ...
                  <section className="rounded-[24px] border border-border/60 bg-card p-5 md:p-6">
                    <div className="mb-4 flex items-start gap-3">
                      ...
                      <div>
                        ...
                        <p className="mt-1 text-sm text-muted-foreground">
                          The live snapshot students and the chatbot read when looking up this building.
                        </p>
                      </div>
                    </div>
                ...
                <div className="grid gap-6 xl:grid-cols-2">
                  <form onSubmit={publishBuildingAnnouncement} className="rounded-[24px] border border-border/60 bg-card p-5 md:p-6">
                    ...
                    <div className="space-y-4">
                      <label className="space-y-2 text-sm font-medium">
                        <span>Title</span>
                        <Input value={announcementDraft.title} onChange={(event) => setAnnouncementDraft((current) => ({ ...current, title: event.target.value }))} placeholder="Student Union closed early" required/>
                      </label>

                      <label className="space-y-2 text-sm font-medium">
                        <span>Message</span>
                        <Textarea value={announcementDraft.message} onChange={(event) => setAnnouncementDraft((current) => ({ ...current, message: event.target.value }))} placeholder="The building will close at 3 PM today due to a water main repair." className="min-h-28 resize-none" required/>
                      </label>

                      <label className="space-y-2 text-sm font-medium">
                        <span>Ends at (optional)</span>
                        <Input type="datetime-local" value={announcementDraft.expiresAt} onChange={(event) => setAnnouncementDraft((current) => ({ ...current, expiresAt: event.target.value }))}/>
                      </label>
                    </div>

                    <Button type="submit" className="mt-5 w-full rounded-2xl" disabled={publishingBuildingId === selectedBuilding.id}>
                  ...
                  <section className="rounded-[24px] border border-border/60 bg-card p-5 md:p-6">
                    ...
                    <div className="space-y-3">
                      {selectedBuilding.announcements.length === 0 ? (<p className="text-sm text-muted-foreground">No active alerts for this building.</p>) : (selectedBuilding.announcements.map((announcement) => (<div key={announcement.id} className="rounded-2xl border border-border/60 bg-muted/10 p-3">
                  ...
                  <form onSubmit={createBuildingEvent} className="rounded-[24px] border border-border/60 bg-card p-5 md:p-6">
                    ...
                    <div className="grid gap-4 md:grid-cols-2">
                      <label className="space-y-2 text-sm font-medium md:col-span-2">
                        ...
                        <Input value={eventDraft.title} onChange={(event) => setEventDraft((current) => ({ ...current, title: event.target.value }))} placeholder="Accessibility open house" required/>
                      </label>

                      <label className="space-y-2 text-sm font-medium md:col-span-2">
                        <span>Description</span>
                        <Textarea value={eventDraft.description} onChange={(event) => setEventDraft((current) => ({ ...current, description: event.target.value }))} placeholder="Walkthrough for students to learn about updated entrances, elevators, and support desks." className="min-h-24 resize-none" required/>
                      </label>

                      <label className="space-y-2 text-sm font-medium">
                        <span>Date</span>
                        <Input type="date" value={eventDraft.date} onChange={(event) => setEventDraft((current) => ({ ...current, date: event.target.value }))} required/>
                      </label>

                      <label className="space-y-2 text-sm font-medium">
                        <span>Time</span>
                        <Input type="time" value={eventDraft.time} onChange={(event) => setEventDraft((current) => ({ ...current, time: event.target.value }))} required/>
                      </label>

                      <label className="space-y-2 text-sm font-medium">
                        <span>Category</span>
                        <select value={eventDraft.category} onChange={(event) => setEventDraft((current) => ({
                ...current,
                category: event.target.value,
            }))} className="flex h-11 w-full rounded-xl border border-input bg-background px-3 text-sm">
                          {eventCategories.map((category) => (<option key={category} value={category}>
                              {category}
                            </option>))}
                        </select>
                      </label>
                      ...
                      <label className="space-y-2 text-sm font-medium">
                        ...
                        <Input type="number" min={1} value={eventDraft.maxAttendees} onChange={(event) => setEventDraft((current) => ({ ...current, maxAttendees: event.target.value }))} placeholder="75"/>
                      </label>

                      <label className="space-y-2 text-sm font-medium md:col-span-2">
                        <span>Location</span>
                        <Input value={eventDraft.location} onChange={(event) => setEventDraft((current) => ({ ...current, location: event.target.value }))} required/>
                      </label>
                    </div>

                    <Button type="submit" className="mt-5 w-full rounded-2xl" disabled={creatingEventBuildingId === selectedBuilding.id}>
                  ...
                  <section className="rounded-[24px] border border-border/60 bg-card p-5 md:p-6">
                    <div className="mb-4 flex items-start gap-3">
                      ...
                      <div>
                        ...
                        <p className="mt-1 text-sm text-muted-foreground">
                          Events tied to this building visible on student calendars.
                        </p>
                      </div>
                    </div>

                    <div className="space-y-3">
                      {selectedBuilding.upcomingEvents.length === 0 ? (<p className="text-sm text-muted-foreground">No upcoming building events yet.</p>) : (selectedBuilding.upcomingEvents.map((event) => (<div key={event.id} className="rounded-2xl border border-border/60 bg-muted/10 p-3">
