...
export function CampusEventsWidget() {
    ...
    return (<div className="space-y-3">
      {events.map(event => (<a href={`/events/${event.id}`} key={event.id} className="flex gap-3.5 p-3 rounded-xl hover:bg-muted/50 transition-all cursor-pointer group/ev">
          {/* Date pill */}
          ...
          <button onClick={(e) => toggleInterest(event.id, e)} className={cn("p-1.5 rounded-lg transition-all self-center", event.isInterested ? "text-red-500" : "text-muted-foreground hover:text-red-500")}>
