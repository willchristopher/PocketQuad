...
import { BellRing, Clock3, Loader2, MapPin, Megaphone, PencilLine, Plus, Save, Send, ShieldCheck, Trash2, UserRoundCheck, } from 'lucide-react';
...
export function FacultyDashboard() {
    ...
    const getErrorMessage = (error, fallback) => error instanceof ApiClientError ? error.message : fallback;
    ...
    const currentTargets = React.useMemo(() => announcementForm.scope === 'BUILDING'
    ...
    return (<div className="space-y-6">
      <section className="relative overflow-hidden rounded-[28px] border border-border/60 bg-card px-6 py-6 md:px-7 md:py-7 animate-in-up">
      ...
      <div className="grid gap-6 xl:grid-cols-[minmax(0,1.2fr)_380px]">
        <section className="space-y-6">
          <section className="rounded-[24px] border border-border/60 bg-card p-5 md:p-6 animate-in-up stagger-2">
            ...
            <form onSubmit={onSubmitStatus} className="space-y-4">
              <div className="grid gap-3 md:grid-cols-3">
                {availabilityOptions.map((option) => (<button key={option.value} type="button" onClick={() => setStatusState((current) => ({ ...current, status: option.value }))} className={cn('rounded-2xl border px-4 py-4 text-left transition-colors', statusState.status === option.value
              ...
              <label className="space-y-2 text-sm font-medium">
                ...
                <Textarea value={statusState.note} onChange={(event) => setStatusState((current) => ({ ...current, note: event.target.value }))} placeholder="Optional context like conference travel, delayed responses, or alternate contact timing." className="min-h-28 resize-none" disabled={statusLoading}/>
              </label>
            ...
            {statusError && (<p className="mt-4 rounded-2xl border border-red-500/30 bg-red-500/10 px-4 py-3 text-sm font-medium text-red-700 dark:text-red-300">
                {statusError}
              </p>)}

            {statusSuccess && (<p className="mt-4 rounded-2xl border border-emerald-500/30 bg-emerald-500/10 px-4 py-3 text-sm font-medium text-emerald-700 dark:text-emerald-300">
                {statusSuccess}
              </p>)}
          </section>

          <section className="rounded-[24px] border border-border/60 bg-card p-5 md:p-6 animate-in-up stagger-3">
            <div className="mb-5 flex items-start gap-3">
            ...
            {officeHoursError && (<p className="mb-4 rounded-2xl border border-red-500/30 bg-red-500/10 px-4 py-3 text-sm font-medium text-red-700 dark:text-red-300">
                {officeHoursError}
              </p>)}

            {officeHoursLoading ? (<p className="text-sm text-muted-foreground">Loading office hours...</p>) : officeHours.length === 0 ? (<div className="rounded-2xl border border-dashed border-border/60 bg-muted/10 px-5 py-8 text-center">
                <p className="text-sm font-medium">No office hours yet</p>
                <p className="mt-1 text-sm text-muted-foreground">Use the form below to add your first slot.</p>
              </div>) : (<div className="grid gap-3">
                {officeHours.map((slot) => (<div key={slot.id} className="rounded-2xl border border-border/60 bg-muted/10 p-4">
          ...
          <section className="rounded-[24px] border border-border/60 bg-card p-5 md:p-6 animate-in-up stagger-4">
            <div className="mb-5 flex items-start gap-3">
            ...
            dayOfWeek: Number(event.target.value),
                    ...
                    {weekdays.map((day, index) => (<option key={day} value={index}>
                        {day}
                      </option>))}
                  </select>
                </label>
            ...
            mode: event.target.value,
              ...
              <div className="flex flex-wrap items-center gap-2">
                ...
                {editingOfficeHourId && (<Button type="button" variant="outline" className="rounded-xl" onClick={resetOfficeHourForm}>
                    Cancel edit
                  </Button>)}
              </div>
            </form>
        ...
        <aside className="space-y-6">
          <section className="rounded-[24px] border border-border/60 bg-card p-5 md:p-6 animate-in-up stagger-3">
            ...
            {announcementsLoading ? (<p className="text-sm text-muted-foreground">Loading announcement access...</p>) : !announcementPermissions ||
            (!announcementPermissions.canPublishCampus &&
                !announcementPermissions.canPublishBuildings &&
                !announcementPermissions.canPublishServices) ? (<div className="rounded-2xl border border-dashed border-border/60 bg-muted/10 px-4 py-6 text-center">
              ...
              </div>) : (<form onSubmit={onSubmitAnnouncement} className="space-y-4">
                <label className="space-y-2 text-sm font-medium">
                  <span>Audience</span>
                ...
                {announcementForm.scope !== 'CAMPUS' && (<label className="space-y-2 text-sm font-medium">
                    ...
                    <select value={announcementForm.targetId} onChange={(event) => setAnnouncementForm((current) => ({ ...current, targetId: event.target.value }))} className="h-10 w-full rounded-xl border border-border/60 bg-background px-3 text-sm" required>
                      {currentTargets.map((target) => (<option key={target.id} value={target.id}>
            ...
            {announcementError && (<p className="mt-4 rounded-2xl border border-red-500/30 bg-red-500/10 px-4 py-3 text-sm font-medium text-red-700 dark:text-red-300">
                {announcementError}
              </p>)}

            {announcementSuccess && (<p className="mt-4 rounded-2xl border border-emerald-500/30 bg-emerald-500/10 px-4 py-3 text-sm font-medium text-emerald-700 dark:text-emerald-300">
                {announcementSuccess}
              </p>)}
            ...
            <div className="mt-6 space-y-3">
              ...
              {!announcementState || announcementState.items.length === 0 ? (<p className="rounded-2xl border border-dashed border-border/60 px-4 py-5 text-sm text-muted-foreground">
                  No announcements published yet.
                </p>) : (<div className="space-y-3">
                  {announcementState.items.slice(0, 4).map((item) => (<div key={item.id} className="rounded-2xl border border-border/60 bg-muted/10 p-4">
                      <div className="flex flex-wrap items-center gap-2">
                        <Badge variant="outline" className="rounded-full">
                          {item.audienceLabel}
                        </Badge>
                      ...
                      {item.linkUrl && (<a href={item.linkUrl} target="_blank" rel="noreferrer" className="mt-2 inline-flex items-center gap-1 text-xs font-semibold text-primary transition-colors hover:text-primary/80">
                          Open link
                        </a>)}
                    </div>))}
                </div>)}
