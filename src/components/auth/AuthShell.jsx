...
export function AuthFieldShell({ icon, trailing, invalid, children, className, }) {
    return (<div className={cn(fieldShellVariants({ invalid }), className)}>
      {icon ? <div className="shrink-0 text-primary/60">{icon}</div> : null}
      {children}
      {trailing ? <div className="shrink-0">{trailing}</div> : null}
    </div>);
}
const messageVariants = cva('rounded-2xl border px-4 py-3 text-sm', {
    variants: {
        variant: {
...
export function AuthMessage({ className, variant, children, }) {
    return (<div className={cn(messageVariants({ variant }), className)}>
      {children}
    </div>);
}
