...
export function AIChatWidget() {
    ...
    const sendMessage = async (overrideInput) => {
        ...
        try {
            ...
            const response = await apiRequest('/api/ai/chat?stream=false', {
                method: 'POST',
                body: {
                    conversationId,
    ...
    if (isMobile) {
        return (<>
        <button onClick={() => setOpen(true)} style={{ bottom: scrollingDown ? 72 : 100 }} className="fixed right-4 z-40 w-12 h-12 rounded-2xl bg-primary text-primary-foreground shadow-xl shadow-primary/30 flex items-center justify-center hover:scale-105 transition-all duration-300" aria-label="Open AI Assistant">
      ...
      {open && (<div className="fixed bottom-20 right-6 z-40 w-[360px] max-w-[90vw] rounded-2xl border border-border/60 bg-card shadow-2xl overflow-hidden">
          ...
          <div ref={listRef} className="max-h-[320px] overflow-y-auto p-4 space-y-3 custom-scrollbar">
            {messages.map((msg) => (<div key={msg.id} className={cn('flex', msg.role === 'user' ? 'justify-end' : 'justify-start')}>
                <div className={cn('max-w-[80%] rounded-2xl px-3.5 py-2.5 text-sm leading-relaxed', msg.role === 'user'
          ...
          <div className="p-4 border-t border-border/60">
            <div className="flex items-center gap-2 rounded-2xl border border-border/60 bg-muted/20 px-3.5 py-2.5">
              ...
              <button onClick={() => {
                void sendMessage();
            }} disabled={!input.trim() || isTyping} className={cn('p-2 rounded-xl transition-all', input.trim() && !isTyping
