...
export function ConfirmDialog({ open, onClose, onConfirm, title, description, confirmLabel = "Confirm", cancelLabel = "Cancel", variant = 'info', checkboxOption, loading = false }) {
    const getIcon = () => {
        switch (variant) {
    ...
    return (<AlertDialog open={open} onOpenChange={onClose}>
      <AlertDialogContent className="rounded-xl sm:rounded-xl">
        ...
        {checkboxOption && (<div className="flex items-center space-x-2 py-2">
                <Checkbox id="confirm-checkbox" checked={checkboxOption.checked} onCheckedChange={(checked) => checkboxOption.onChange(checked)}/>
        ...
        <AlertDialogFooter>
          ...
          <AlertDialogAction onClick={(e) => {
            e.preventDefault();
            onConfirm();
        }} disabled={loading} className={getButtonClass()}>
