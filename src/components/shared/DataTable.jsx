...
export function DataTable({ columns, data, searchKey, searchPlaceholder = "Filter...", bulkActions, loading = false, pagination = true, pageSize = 10, onRowClick, selectable = false, }) {
    ...
    const selectedIds = Object.keys(rowSelection).map(index => {
        ...
        return data[parseInt(index)]?.id; // Unsafe assumption, but common.
    }).filter(Boolean);
    return (<div className="w-full space-y-4">
      <div className="flex items-center justify-between">
        <div className="flex flex-1 items-center space-x-2">
          {searchKey && (<Input placeholder={searchPlaceholder} value={table.getColumn(searchKey)?.getFilterValue() ?? ""} onChange={(event) => table.getColumn(searchKey)?.setFilterValue(event.target.value)} className="max-w-sm" disabled={loading}/>)}
          {Object.keys(rowSelection).length > 0 && bulkActions && (<div className="flex items-center space-x-2 ml-4 animate-in fade-in slide-in-from-left-4 duration-200">
               <span className="text-sm text-muted-foreground mr-2 border-r pr-4">
                  {Object.keys(rowSelection).length} selected
               </span>
               {bulkActions.map((action, i) => (<Button key={i} size="sm" variant={action.variant || "secondary"} onClick={() => action.onClick(selectedIds)}>
        ...
        <Table>
          ...
          <TableBody>
            {loading ? (<TableRow>
               ...
               </TableRow>) : table.getRowModel().rows?.length ? (table.getRowModel().rows.map((row) => (<TableRow key={row.id} data-state={row.getIsSelected() && "selected"} className={cn(onRowClick && "cursor-pointer hover:bg-muted/50")} onClick={() => onRowClick && onRowClick(row.original)}>
