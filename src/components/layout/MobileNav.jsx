...
export function MobileNav() {
  ...
  return (
    <div className="fixed inset-x-0 bottom-0 z-50 px-3 pb-3 lg:hidden">
      ...
      <nav
        className={cn(
          'panel-card-elevated mx-auto flex max-w-xl items-center justify-between rounded-2xl px-2 transition-all duration-300',
        ...
        {studentMobileNavigationItems.map((item) => {
          ...
          return (
            <Link
              ...
              href={item.href}
              className={cn(
                'relative flex min-w-[64px] flex-1 flex-col items-center justify-center rounded-xl px-2 py-2 text-center transition-all duration-300',
                isActive ? 'text-primary' : 'text-muted-foreground',
              )}
            >
              {isActive ? <span className="absolute inset-0 rounded-xl bg-primary/10" /> : null}
