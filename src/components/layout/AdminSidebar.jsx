...
import { getAllowedAdminTabs } from '@/lib/auth/portalPermissions';
...
export function AdminSidebar({ className, mobile = false, onNavigate }) {
    ...
    const allowedTabs = React.useMemo(() => {
        if (!profile)
            return adminLinks.map((item) => item.tab);
        const resolved = getAllowedAdminTabs(profile);
        return resolved.length > 0 ? resolved : ['overview'];
    }, [profile]);
    const visibleLinks = React.useMemo(() => adminLinks.filter((item) => allowedTabs.includes(item.tab)), [allowedTabs]);
    ...
    const buildAdminHref = React.useCallback((tab) => {
        const params = new URLSearchParams(searchParams.toString());
        params.set('tab', tab);
    ...
    return (<aside className={cn(mobile
      ...
      <nav className="flex-1 space-y-0.5 overflow-y-auto p-3 custom-scrollbar">
        {visibleLinks.map((item) => {
            ...
            return (<Link key={item.tab} href={href} onClick={onNavigate} className={cn('group relative flex items-center gap-3 rounded-lg px-3 py-2 text-[13px] font-medium transition-colors duration-150', isActive
      ...
      <div className="border-t border-border/50 p-3 space-y-2">
        ...
        <Button variant="outline" className="w-full justify-start gap-2 rounded-lg text-xs h-9" onClick={handleChangeUniversity} disabled={!selectedUniversityId}>
          <School className="h-3.5 w-3.5"/>
          Change university
        </Button>
        <Button variant="outline" className="w-full justify-start gap-2 rounded-lg text-xs h-9" onClick={handleLogout}>
