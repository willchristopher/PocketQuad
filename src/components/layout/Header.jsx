...
import { Bell, BellRing, CalendarClock, CalendarRange, ExternalLink, Flag, LayoutGrid, LogOut, MapPinned, Menu, MessageCircleMore, Moon, Palette, Search, Sun, UserCircle2, Users2, } from 'lucide-react';
...
export function Header() {
    ...
    return (<header className="sticky top-0 z-30 flex h-14 w-full items-center justify-between gap-4 border-b border-border/50 bg-background/80 px-4 backdrop-blur-xl md:px-6">
      <div className="flex min-w-0 items-center gap-2">
        ...
        <Sheet open={sheetOpen} onOpenChange={setSheetOpen}>
          <SheetTrigger asChild>
            <button className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg text-muted-foreground transition-colors hover:bg-muted/50 hover:text-foreground md:hidden" aria-label="Open navigation menu">
          ...
          <SheetContent side="left" className="flex w-[280px] flex-col p-0">
            ...
            <nav className="flex-1 overflow-y-auto px-2 py-2">
              {navSections.map((section, i) => (<div key={i} className="mb-1">
                  {section.title && (<p className="px-3 py-1 text-[10px] font-semibold uppercase tracking-wider text-muted-foreground/60">
                      {section.title}
                    </p>)}
                ...
                return (<Link key={item.href} href={item.href} onClick={() => setSheetOpen(false)} className={cn('flex items-center gap-3 rounded-lg px-3 py-2 text-sm font-medium transition-colors', isActive
            ...
            return (<Link key={item.href} href={item.href} onClick={() => setSheetOpen(false)} className={cn('flex items-center gap-3 rounded-lg px-3 py-2 text-sm font-medium transition-colors', isActive
              ...
              <button onClick={() => void handleSignOut()} className="flex w-full items-center gap-3 rounded-lg px-3 py-2 text-sm font-medium text-muted-foreground transition-colors hover:bg-muted/50 hover:text-foreground">
      ...
      <div className="flex items-center gap-1.5">
        ...
        <button onClick={() => document.dispatchEvent(new KeyboardEvent('keydown', { key: 'k', metaKey: true, bubbles: true }))} className="flex h-8 w-8 items-center justify-center rounded-lg text-muted-foreground transition-colors hover:bg-muted/50 hover:text-foreground md:hidden" aria-label="Search">
          <Search className="h-4 w-4"/>
        </button>

        <button onClick={() => {
            const cycle = universityColors
                ? ['light', 'dark', 'university']
                : ['light', 'dark'];
            const idx = cycle.indexOf(themeMode);
            setThemeMode(cycle[(idx + 1) % cycle.length]);
        }} className="flex h-8 w-8 items-center justify-center rounded-lg text-muted-foreground transition-colors hover:bg-muted/50 hover:text-foreground" aria-label={`Theme: ${themeMode}. Click to switch.`} title={!mounted
            ? 'Toggle theme'
            : themeMode === 'light'
                ? 'Light mode'
                : themeMode === 'dark'
          ...
          {!mounted ? (<Sun className="h-4 w-4"/>) : themeMode === 'light' ? (<Sun className="h-4 w-4"/>) : themeMode === 'dark' ? (<Moon className="h-4 w-4"/>) : (<Palette className="h-4 w-4"/>)}
        </button>
